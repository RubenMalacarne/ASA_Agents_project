...
class Parcels_DataBase{
    ...
    */
    constructor(client){
        this.client = client;
        client.onParcelsSensing((parcels)=>{this.updateParcelsData(parcels)})
    }
    ...
    updateLostParcels(my_data){
        ...
        for(let [id,data] of this.database){
            if( ! update_ids.includes(id)){
                if(manhattan_distance(my_data, data) <= VIEW_DISTANCE ){ // if a parcel I should be able to see has disappeared forget it
